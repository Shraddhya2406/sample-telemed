...
<div class="grid min-h-[calc(100vh-9rem)] gap-5 pb-20 lg:grid-cols-[minmax(0,1fr)_22rem] lg:pb-0">
    <section class="flex min-h-[38rem] flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="border-b border-slate-200 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-950 sm:px-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div class="min-w-0">
                    <div class="flex items-center gap-3">
                        <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-blue-600 text-white">
                        ...
                        <div class="min-w-0">
                            <h2 class="truncate text-lg font-bold text-slate-950 dark:text-white">Clinical AI Assistant</h2>
                            <p class="text-sm text-slate-500 dark:text-slate-400">One question at a time, saved for doctor review.</p>
                        </div>
                    </div>
                </div>
                <button type="button" id="new-assessment" class="inline-flex items-center justify-center gap-2 rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm font-bold text-blue-700 transition hover:bg-blue-50 dark:border-slate-700 dark:bg-slate-900 dark:text-blue-300 dark:hover:bg-slate-800">
            ...
            <div class="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900 dark:border-amber-900/70 dark:bg-amber-950/40 dark:text-amber-100">
        ...
        <div id="chat-messages" class="flex-1 space-y-4 overflow-y-auto bg-slate-50 px-4 py-5 dark:bg-slate-950 sm:px-5" aria-live="polite"></div>

        <div id="typing-indicator" class="hidden border-t border-slate-200 bg-white px-5 py-3 dark:border-slate-800 dark:bg-slate-900">
        ...
        <form id="chat-form" class="border-t border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 sm:p-5">
            <div class="flex gap-2">
                <label for="message-input" class="sr-only">Message</label>
                <textarea id="message-input" class="min-h-12 flex-1 resize-none rounded-lg border border-slate-200 bg-white px-3 py-3 text-sm leading-5 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:ring-blue-950" rows="1" placeholder="Describe your symptoms" required></textarea>
            ...
            <div class="mt-3 flex flex-wrap items-center justify-between gap-2">
                <p id="chat-error" class="hidden text-sm font-semibold text-rose-600"></p>
                <button type="button" id="complete-assessment" class="ml-auto inline-flex items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-700 transition hover:bg-emerald-100 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-300">Finish and Save Summary</button>
            </div>
        </form>
    </section>
    ...
    <aside class="space-y-5">
        ...
        <section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div id="history-list" class="mt-3 max-h-[28rem] space-y-2 overflow-y-auto pr-1"></div>
        </section>
    </aside>
</div>
@endsection
            ...
            messagesEl.innerHTML = messages.map((message) => {
                ...
                return `
                    <div class="flex ${isPatient ? 'justify-end' : 'justify-start'}">
                        <div class="max-w-[84%] rounded-lg px-4 py-3 shadow-sm ${isPatient ? 'bg-blue-600 text-white' : 'border border-slate-200 bg-white text-slate-800 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100'}">
